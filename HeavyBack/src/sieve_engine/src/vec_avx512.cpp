...
static inline __mmask64 lattice_mask64(int count) {
    if (count <= 0) return (__mmask64)0;
    if (count >= 64) return ~(__mmask64)0;
    return (((__mmask64)1) << count) - 1;
}

void vec_collect(int8_t *dst, int8_t **src_list, int n, int CSD, int CSD16) {
    const __mmask64 m0 = lattice_mask64(CSD);
    const __mmask64 m1 = lattice_mask64(CSD - 64);
    const __mmask64 m2 = lattice_mask64(CSD - 128);

#if BGJL_HOST_UPK
    const __mmask64 wm0 = lattice_mask64(CSD16);
    const __mmask64 wm1 = lattice_mask64(CSD16 - 64);
    const __mmask64 wm2 = lattice_mask64(CSD16 - 128);
#endif

    for (int j = 0; j < n; ++j) {
        int8_t *out = dst + j * (BGJL_HOST_UPK ? CSD16 : CSD);
        const int8_t *in = src_list[j];

        const __m512i v0 = _mm512_maskz_loadu_epi8(m0, in);
        const __m512i v1 = _mm512_maskz_loadu_epi8(m1, in + 64);
        const __m512i v2 = _mm512_maskz_loadu_epi8(m2, in + 128);

        _mm512_mask_storeu_epi8(out,       (BGJL_HOST_UPK ? wm0 : m0), v0);
        _mm512_mask_storeu_epi8(out + 64,  (BGJL_HOST_UPK ? wm1 : m1), v1);
        _mm512_mask_storeu_epi8(out + 128, (BGJL_HOST_UPK ? wm2 : m2), v2);
    }
}
