...
static inline __mmask64 lattice_pool_mask64(int count) {
...
extern int pop_check_vec_err(int8_t *src1, int8_t *src2, int CSD) {
    const __mmask64 masks[3] = {
        lattice_pool_mask64(CSD),
        lattice_pool_mask64(CSD - 64),
        lattice_pool_mask64(CSD - 128)
    };
    ...
    for (int block = 0; block < 3; ++block) {
        const __mmask64 mask = masks[block];
        if (!mask) continue;

        const int offset = block * 64;
        const __m512i x = _mm512_maskz_loadu_epi8(mask, src1 + offset);
        const __m512i y = _mm512_maskz_loadu_epi8(mask, src2 + offset);

        const __m512i xy = _mm512_subs_epi8(x, y);
        const __m512i yx = _mm512_subs_epi8(y, x);
        const __m512i d = _mm512_max_epi8(xy, yx);
        _mm512_store_si512(reinterpret_cast<__m512i *>(tmp), d);

        const int valid = CSD - offset >= 64 ? 64 : CSD - offset;
        for (int i = 0; i < valid; ++i) {
            const int diff = static_cast<unsigned char>(tmp[i]);
            if (diff > vec_err) {
                vec_err = diff;
