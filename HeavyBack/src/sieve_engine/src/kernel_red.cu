...
__global__ void _reduce_kernel(int *out, int *num_out, int out_max_size, 
                                      const int8_t *__restrict__ vec_pad16,
...
#define INSTANTIATE(layer, CSD16, sbuc_freq, gbuc_freq) \
    template __global__ void _reduce_kernel<layer, CSD16, sbuc_freq, gbuc_freq>(int *out, int *num_out, int out_max_size, \
                 const int8_t *__restrict__ vec_pad16, int32_t *__restrict__ vnorm, int *n_ptr, int32_t goal_norm)


template __global__ void _reduce_prepare<0>(int32_t *vnorm, int *n_ptr, int max_n);
template __global__ void _reduce_prepare<1>(int32_t *vnorm, int *n_ptr, int max_n);

INSTANTIATE(1, 176, 64, 512);
INSTANTIATE(0, 176, 16, 128);
INSTANTIATE(1, 176, 16, 128);
INSTANTIATE(0, 176, 1, 2);
INSTANTIATE(1, 176, 1, 2);
#if RED_MIN_CSD16 < 176
INSTANTIATE(1, 160, 64, 512);
INSTANTIATE(0, 160, 16, 128);
INSTANTIATE(1, 160, 16, 128);
INSTANTIATE(0, 160, 1, 2);
INSTANTIATE(1, 160, 1, 2);
#endif
#if RED_MIN_CSD16 < 160
INSTANTIATE(1, 144, 64, 512);
INSTANTIATE(0, 144, 16, 128);
INSTANTIATE(1, 144, 16, 128);
INSTANTIATE(0, 144, 1, 2);
INSTANTIATE(1, 144, 1, 2);
#endif
#if RED_MIN_CSD16 < 144
INSTANTIATE(1, 128, 64, 512);
INSTANTIATE(0, 128, 16, 128);
INSTANTIATE(1, 128, 16, 128);
INSTANTIATE(0, 128, 1, 2);
INSTANTIATE(1, 128, 1, 2);
#endif